...
export function SignalPresaleBanner({ variant = "full", className = "" }: SignalPresaleBannerProps) {
  ...
  if (variant === "inline") {
    return (
      <GlassCard variant="feature" className={`rounded-xl gradient-border hover-elevate overflow-hidden ${className}`}>
        <a
          href={presaleUrl}
          target="_blank"
          rel="noopener noreferrer"
          className="block p-4 group transition-all"
  ...
  if (variant === "compact") {
    return (
      <GlassCard variant="feature" className={`rounded-xl lg:rounded-2xl gradient-border hover-elevate overflow-hidden ${className}`}>
        <a
          href={presaleUrl}
          target="_blank"
          rel="noopener noreferrer"
          ...
          <div className="flex flex-col lg:flex-row lg:items-center gap-5 lg:gap-8">
            <div className="flex-1">
              ...
              <p className="text-sm text-muted-foreground mb-4 max-w-xl">
                Signal is the native digital asset powering a 25-app ecosystem with 1.97M+ lines of production code. 
