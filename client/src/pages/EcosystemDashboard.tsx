...
function StatusDot({ status }: { status: string }) {
  ...
  return <div className={`w-2.5 h-2.5 rounded-full shadow-[0_10px_30px_rgba(0,0,0,0.5)] ${colors[status] || colors.checking}`} />;
}
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={fetchHealth}
              disabled={loading}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium bg-white/5 hover:bg-white/10 border border-white/[0.08] transition-colors disabled:opacity-40"
            ...
            <Link
              href="/command"
              className="flex items-center gap-1.5 bg-white/5 border border-white/[0.08] text-[#050505] px-3 py-1.5 rounded-lg text-sm font-semibold hover:opacity-90 transition-opacity"
        ...
        >
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-6 items-center">
            ...
            <div>
              ...
              <p className="text-[#888] mt-2 max-w-2xl mx-auto">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  ...
                  { label: "Avg Response", value: `${summary.avgResponseTime}ms`, icon: Timer, color: "text-[#888]", bg: "bg-white/5 border-white/[0.08]" },
                ].map((stat, i) => (
                  <motion.div
                    key={stat.label}
                    initial={{ opacity: 0, y: 10 }}
            ...
            {quickActions.map((action, i) => (
              <motion.div key={action.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 + i * 0.04 }}>
                <Link href={action.href} data-testid={`quick-${action.label.toLowerCase().replace(/\s+/g, '-')}`}>
                  <GlassCard className="p-3 rounded-xl group hover:border-white/[0.1] transition-all cursor-pointer h-full">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 mb-4">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full bg-white/[0.03] border border-white/[0.08] rounded-lg pl-9 pr-4 py-2 text-sm text-[#050505] placeholder-gray-500 focus:border-white/[0.08] focus:outline-none transition-colors"
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm border transition-colors ${
                showFilters ? "bg-primary/10 border-primary/30 text-primary" : "bg-white/[0.03] border-white/[0.08] text-muted-foreground hover:text-[#050505]"
              }`}
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      <button
                        ...
                        className={`px-2.5 py-1 rounded-md text-xs font-medium transition-colors ${
                          filterCategory === "all" ? "bg-primary/20 text-primary" : "bg-white/5 text-muted-foreground hover:text-[#050505]"
                        }`}
                      ...
                      {categories.map(cat => (
                        <button
                          ...
                          className={`px-2.5 py-1 rounded-md text-xs font-medium transition-colors ${
                            filterCategory === cat ? "bg-primary/20 text-primary" : "bg-white/5 text-muted-foreground hover:text-[#050505]"
                          }`}
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {["all", "online", "degraded", "offline", "timeout"].map(s => (
                        <button
                          ...
                          className={`px-2.5 py-1 rounded-md text-xs font-medium transition-colors ${
                            filterStatus === s ? "bg-primary/20 text-primary" : "bg-white/5 text-muted-foreground hover:text-[#050505]"
                          }`}
            ...
            filteredApps.map(app => {
              ...
              return (
                <motion.div key={app.id} variants={itemVariants}>
                  <GlassCard
                    className={`p-4 rounded-xl group hover:border-white/[0.1] transition-all duration-300 relative overflow-hidden ${
              ...
              {[
                ...
                tool.ext ? (
                  <a key={tool.label} href={tool.href} target="_blank" rel="noopener noreferrer" data-testid={`tool-${tool.label.toLowerCase().replace(/\s+/g, '-')}`}>
                    <GlassCard variant="stat" className="p-4 rounded-xl group hover:border-white/[0.1] transition-all cursor-pointer h-full">
