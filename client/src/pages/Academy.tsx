...
function CarouselControls({ index, itemCount, scroll }: { index: number; itemCount: number; scroll: (d: "left" | "right") => void }) {
  return (
    <div className="flex items-center justify-center gap-6 mt-8">
      <button
        className="w-11 h-11 rounded-full border border-white/[0.08] bg-white/5 backdrop-blur-sm flex items-center justify-center text-[#666] dark:text-[#050505]/50 hover:text-[#888] hover:border-white/[0.08] hover:bg-white/5 transition-all duration-300"
        onClick={() => scroll("left")}
        aria-label="Previous"
      >
      ...
      <div className="flex items-center gap-2">
        {Array.from({ length: itemCount }).map((_, i) => (
          <div
            key={i}
            className={`h-1.5 rounded-full transition-all duration-300 ${i === index ? "w-6 bg-cyan-400 shadow-[0_0_10px_rgba(6,182,212,0.6)]" : "w-1.5 bg-[#0a0a0a]/15"}`}
          />
        ))}
      </div>
      <button
...
function LevelBadge({ level }: { level: string }) {
  const colors: Record<string, string> = {
    ...
    Advanced: "text-[#888] bg-white/5 border-white/[0.08]",
  };
  return (
    <span className={`px-2.5 py-1 rounded-full border text-[10px] font-semibold uppercase tracking-wider ${colors[level] || "text-[#888] bg-white/5 border-white/[0.08]"}`}>
      {level}
    </span>
  );
}
    ...
    <div className="min-h-screen overflow-x-hidden transition-colors duration-300" style={{ background: 'var(--nav-bg, #06060a)', color: 'var(--text-primary, #fff)' }}>
      ...
      <header className="sticky top-0 z-50 backdrop-blur-xl border-b transition-colors duration-300" style={{ background: 'var(--nav-bg, rgba(6,6,10,0.9))', borderColor: 'var(--glass-border, rgba(255,255,255,0.1))' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link href="/" className="text-[#888] hover:text-[#888] transition-colors"><ChevronLeft className="w-5 h-5" /></Link>
            <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2">
            <ThemeToggle />
            <Link href="/lume" className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium bg-white/5 hover:bg-white/10 border border-white/[0.08] transition-colors text-gray-300 hover:text-[#050505]">
      ...
      <section className="relative min-h-[85vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute inset-0">
          ...
          <div className="absolute inset-0 bg-[#0a0a0a]/80 dark:bg-transparent transition-colors duration-300" />
        ...
        <div className="relative z-10 max-w-5xl mx-auto text-center px-4 pt-20 pb-16">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border backdrop-blur-sm mb-8 transition-colors duration-300" style={{ background: 'var(--badge-bg)', borderColor: 'var(--glass-border)' }}>
              <GraduationCap className="w-4 h-4 text-cyan-600 dark:text-[#888]" />
          ...
          <motion.div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-12" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.4 }}>
            <a href="#paths" className="group inline-flex items-center gap-2 px-8 py-3.5 rounded-xl bg-white/5 border border-white/[0.08] dark:from-cyan-500 dark:to-teal-500 text-white dark:text-[#050505] font-semibold hover:from-cyan-500 hover:to-teal-500 transition-all duration-300 shadow-[0_10px_30px_rgba(0,0,0,0.5)] shadow-[0_10px_30px_rgba(255,255,255,0.05)] relative overflow-hidden">
            ...
            {stats.map((stat, i) => (
              <div key={stat.label} className="flex items-center">
                ...
                <div className="flex flex-col items-center px-4">
                  <stat.icon className="w-4 h-4 text-cyan-600 dark:text-[#888]/60 mb-1" />
                  <span className="text-xl sm:text-2xl font-bold font-mono bg-white/5 border border-white/[0.08] dark:from-cyan-400 dark:to-teal-400 bg-clip-text text-transparent">{stat.value}</span>
        ...
        <div className="max-w-7xl mx-auto px-4 mb-12">
          <motion.div className="text-center" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <h2 className="text-3xl md:text-5xl font-bold mb-4">
              <span className="bg-white/5 border border-white/[0.08] bg-clip-text text-transparent">Learning Paths</span>
            </h2>
            <p className="text-[#888] dark:text-[#050505]/35 max-w-2xl mx-auto">
        ...
        >
          {learningPaths.map((path) => (
            <div
              key={path.id}
              data-card
              className="flex-shrink-0 w-[320px] sm:w-[380px] lg:w-[420px] rounded-2xl overflow-hidden border border-white/[0.08] dark:border-white/[0.08] bg-[#0a0a0a]/80 dark:bg-[rgba(12,18,36,0.65)] backdrop-blur-2xl group cursor-pointer transition-all duration-500 hover:border-white/[0.08] hover:shadow-[0_0_50px_rgba(6,182,212,0.1)]"
            ...
            >
              ...
              <div className="relative h-48 overflow-hidden">
                ...
                <div className="absolute top-4 left-4">
                  <div className="w-10 h-10 rounded-xl bg-black/40 backdrop-blur-sm border border-white/[0.08] flex items-center justify-center">
                    <path.icon className="w-5 h-5 text-[#888]" />
                  </div>
                </div>
              ...
              <div className="p-6">
                ...
                <p className="text-[#888] dark:text-[#050505]/35 text-sm mb-5 leading-relaxed line-clamp-2">{path.description}</p>
                <div className="flex items-center gap-4 text-xs text-[#555] dark:text-[#050505]/25">
        ...
        >
          {featuredCourses.map((course) => (
            <div
              key={course.id}
              data-card
              className="flex-shrink-0 w-[300px] sm:w-[360px] lg:w-[400px] rounded-2xl overflow-hidden border border-white/[0.08] dark:border-white/[0.08] bg-[#0a0a0a]/80 dark:bg-[rgba(12,18,36,0.65)] backdrop-blur-2xl group cursor-pointer transition-all duration-500 hover:border-white/[0.08] hover:shadow-[0_0_50px_rgba(6,182,212,0.1)]"
              style={{ scrollSnapAlign: "start" }}
            >
              {/* Image Header */}
              <div className="relative h-44 overflow-hidden">
                ...
                <div className="absolute bottom-4 left-5 right-5 flex items-end justify-between">
                  <div className="flex items-center gap-2">
                    <div className="w-9 h-9 rounded-lg bg-black/40 backdrop-blur-sm border border-white/[0.08] flex items-center justify-center">
                      <course.icon className="w-4 h-4 text-[#888]" />
                    </div>
                    <span className="text-[10px] text-[#888]/70 font-medium uppercase tracking-wider">{course.path}</span>
                  </div>
              ...
              <div className="p-5">
                ...
                <p className="text-[#888] dark:text-[#050505]/35 text-sm mb-4 leading-relaxed line-clamp-2">{course.description}</p>
                <div className="flex items-center gap-4 text-xs text-[#555] dark:text-[#050505]/25">
      ...
      <section className="py-24 px-4">
        <div className="max-w-7xl mx-auto">
          <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/[0.08] mb-6">
                    ...
                    <ul className="space-y-3">
                      {tier.topics.map((topic) => (
                        <li key={topic} className="flex items-center gap-3 text-sm text-[#050505]/60">
                          <CheckCircle2 className="w-4 h-4 text-[#888] shrink-0" />
                          <span>{topic}</span>
                        </li>
                      ))}
                    </ul>
          ...
          <motion.div className="mt-10 text-center" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>
            <div className="inline-flex flex-wrap items-center justify-center gap-3 sm:gap-0 px-6 py-3 rounded-xl bg-white/5 border border-white/[0.08] backdrop-blur-sm">
              {[{ v: "316", l: "tests", c: "text-[#888]" }, { v: "12,215", l: "LOC", c: "text-teal-400" }, { v: "90%", l: "less code", c: "text-sky-400" }].map((s, i) => (
                <div key={s.l} className="flex items-center">
                  ...
                  <div className="flex items-center gap-2 text-xs sm:text-sm">
                    ...
                    <span className="text-[#888] dark:text-[#050505]/35">{s.l}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        >
          {ecosystemSkills.map((skill) => (
            <div
              key={skill.skill}
              data-card
              className="flex-shrink-0 w-[280px] sm:w-[320px] rounded-2xl overflow-hidden border border-white/[0.08] dark:border-white/[0.08] bg-[#0a0a0a]/80 dark:bg-[rgba(12,18,36,0.65)] backdrop-blur-2xl p-6 group transition-all duration-500 hover:border-white/[0.08] hover:shadow-[0_0_40px_rgba(6,182,212,0.08)]"
              style={{ scrollSnapAlign: "start" }}
            >
              <div className="flex items-center gap-3 mb-5">
                <div className="w-10 h-10 rounded-xl bg-white/5 border border-white/[0.08] flex items-center justify-center">
                  <skill.icon className="w-5 h-5 text-[#888]" />
                </div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {skill.apps.map((app) => (
                  <span key={app} className="px-2.5 py-1 rounded-lg bg-white/5 border border-white/[0.06] text-[11px] text-[#888] dark:text-[#050505]/45 font-medium">
                    {app}
                  </span>
                ))}
              </div>
        ...
        >
          {certifications.map((cert) => (
            <div
              key={cert.title}
              data-card
              className="flex-shrink-0 w-[240px] sm:w-[260px] rounded-2xl overflow-hidden border border-white/[0.08] dark:border-white/[0.08] bg-[#0a0a0a]/80 dark:bg-[rgba(12,18,36,0.65)] backdrop-blur-2xl p-6 text-center group transition-all duration-500 hover:border-white/[0.08] hover:shadow-[0_0_50px_rgba(6,182,212,0.12)]"
              style={{ scrollSnapAlign: "start" }}
            >
              {/* Icon Ring */}
              <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-cyan-500/10 to-teal-500/10 border border-white/[0.08] flex items-center justify-center mx-auto mb-4 group-hover:border-white/[0.08] transition-colors">
                <cert.icon className="w-7 h-7 text-[#888]" />
              </div>
              <div className="inline-flex px-3 py-1 rounded-full bg-gradient-to-r from-cyan-500/15 to-teal-500/15 border border-white/[0.08] mb-3">
              ...
              <p className="text-[#888] dark:text-[#050505]/35 text-xs leading-relaxed mb-4">{cert.description}</p>
              <div className="flex items-center justify-center gap-1.5 text-[10px] text-[#888]/50">
            ...
            {[
              ...
              <motion.div key={item.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                <GlassCard glow className="p-6 h-full">
                  <div className="w-12 h-12 rounded-xl bg-white/5 border border-white/[0.08] flex items-center justify-center mb-4">
                    <item.icon className="w-6 h-6 text-cyan-600 dark:text-[#888]" />
                  </div>
                  ...
                  <div className="flex items-center gap-2 text-xs" style={{ color: 'var(--text-dim)' }}>
                    ...
                    {'link' in item ? (
                      <a href={item.link} target="_blank" rel="noopener noreferrer" className="hover:text-cyan-600 dark:hover:text-[#888] transition-colors">{item.meta}</a>
                    ) : (
                      <span>{item.meta}</span>
                    )}
                  </div>
          ...
          <motion.div className="flex flex-col md:flex-row gap-12 items-center" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            <div className="flex-1">
              ...
              <div className="flex items-center gap-4">
                <a href="https://strata.tlid.io" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl border font-medium transition-all hover:border-white/[0.08]" style={{ background: 'var(--badge-bg)', borderColor: 'var(--glass-border)', color: 'var(--text-primary)' }}>
                  <Archive className="w-4 h-4 text-cyan-600 dark:text-[#888]" /> View Strata Hub
                </a>
              ...
              <GlassCard className="relative p-8 border-sky-500/20">
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <div key={p.doi} className="flex flex-col gap-1 p-3 rounded-xl hover:bg-white/5 border border-transparent hover:border-white/[0.05] transition-colors cursor-pointer">
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-semibold text-[#050505]/90">{p.title}</span>
                        <span className="text-[10px] px-2 py-0.5 rounded bg-white/5 text-[#050505]/30">{p.doi}</span>
                      </div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <GlassCard className="p-8 text-center flex flex-col h-full">
              ...
              <ul className="text-sm text-left space-y-4 mb-8 flex-grow">
                <li className="flex items-start gap-3"><CheckCircle2 className="w-5 h-5 text-[#050505]/30 mt-0.5 shrink-0" /> <span className="text-[#666] dark:text-[#050505]/50">Access to Web Dev 101</span></li>
                <li className="flex items-start gap-3"><CheckCircle2 className="w-5 h-5 text-[#050505]/30 mt-0.5 shrink-0" /> <span className="text-[#666] dark:text-[#050505]/50">Lume Syntax Fundamentals</span></li>
                <li className="flex items-start gap-3"><CheckCircle2 className="w-5 h-5 text-[#050505]/30 mt-0.5 shrink-0" /> <span className="text-[#666] dark:text-[#050505]/50">Read-only Signal Chat access</span></li>
            ...
            {/* Pro Builder */}
            <GlassCard glow className="p-8 text-center flex flex-col h-full relative border-white/[0.08]">
              <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                <div className="bg-white/5 border border-white/[0.08] text-black text-xs font-bold uppercase tracking-wider py-1 px-4 rounded-full shadow-[0_10px_30px_rgba(0,0,0,0.5)] shadow-[0_10px_30px_rgba(255,255,255,0.05)] relative overflow-hidden">
              ...
              <ul className="text-sm text-left space-y-4 mb-8 flex-grow">
                <li className="flex items-start gap-3"><CheckCircle2 className="w-5 h-5 text-[#888] mt-0.5 shrink-0" /> <span className="text-[#050505]/80">All 60+ Advanced Courses</span></li>
                <li className="flex items-start gap-3"><CheckCircle2 className="w-5 h-5 text-[#888] mt-0.5 shrink-0" /> <span className="text-[#050505]/80">Trust Layer Architecture Guides</span></li>
                <li className="flex items-start gap-3"><CheckCircle2 className="w-5 h-5 text-[#888] mt-0.5 shrink-0" /> <span className="text-[#050505]/80">AI Content Generator Limits</span></li>
                <li className="flex items-start gap-3"><CheckCircle2 className="w-5 h-5 text-[#888] mt-0.5 shrink-0" /> <span className="text-[#050505]/80">Full Signal Chat Support</span></li>
              </ul>
              <Button className="w-full bg-white/5 border border-white/[0.08] text-black hover:opacity-90 font-semibold" onClick={() => handleSubscribe("pro-builder")}>Subscribe Now</Button>
            </GlassCard>
        ...
        <div className="max-w-4xl mx-auto">
          <GlassCard glow className="p-8 sm:p-12 md:p-16 text-center relative overflow-hidden">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="relative z-10">
              <GraduationCap className="w-12 h-12 text-[#888] mx-auto mb-6" />
              <h2 className="text-3xl md:text-4xl font-bold mb-4">
                Start Your <span className="bg-white/5 border border-white/[0.08] bg-clip-text text-transparent">Learning Journey</span>
              </h2>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                <a href="#paths" className="group inline-flex items-center gap-2 px-8 py-3.5 rounded-xl bg-white/5 border border-white/[0.08] text-black font-semibold hover:from-cyan-400 hover:to-teal-400 transition-all duration-300 shadow-[0_10px_30px_rgba(0,0,0,0.5)] shadow-[0_10px_30px_rgba(255,255,255,0.05)] relative overflow-hidden">
                  <span className="absolute inset-0 bg-gradient-to-r from-transparent via-white/25 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-700" />
                  <Rocket className="w-4 h-4 relative z-10" />
                  <span className="relative z-10">Enroll Now</span>
                </a>
                <Link href="/lume" className="inline-flex items-center gap-2 px-8 py-3.5 rounded-xl border border-white/[0.08] bg-white/5 backdrop-blur-sm text-white dark:text-[#050505] font-medium hover:bg-white/10 hover:border-white/[0.1] transition-all duration-300">
                  <Terminal className="w-4 h-4" /> Explore Lume
                </Link>
              </div>
              <div className="mt-8 flex flex-wrap items-center justify-center gap-6 text-xs text-[#555] dark:text-[#050505]/25">
                <a href="https://academy.tlid.io" target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 hover:text-[#888] transition-colors">
                  <ExternalLink className="w-3 h-3" /> academy.tlid.io
                </a>
                <Link href="/explore" className="flex items-center gap-1 hover:text-[#888] transition-colors">
      ...
      <Dialog open={showSubscribe} onOpenChange={setShowSubscribe}>
        <DialogContent className="glass-card border-white/[0.08]">
          ...
          <div className="space-y-4">
            <div>
              <label className="text-sm text-[#666] dark:text-[#050505]/40 mb-2 block">Email</label>
              <Input type="email" value={subscribeEmail} onChange={(e) => setSubscribeEmail(e.target.value)} placeholder="developer@ecosystem.io" />
            </div>
            <div>
              <label className="text-sm text-[#666] dark:text-[#050505]/40 mb-2 block">Company / Agency (Optional)</label>
