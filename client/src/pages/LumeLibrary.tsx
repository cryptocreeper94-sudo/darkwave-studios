...
function StatusMonitorWidget() {
  ...
  const statusIcon = (s: string) => {
    ...
    return <RefreshCw className="w-3 h-3 text-[#888] animate-spin" />;
  };

  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2 text-xs text-[#050505]/50">
      ...
      {services.map((s, i) => (
        <div key={i} className={`flex items-center justify-between py-1.5 px-2.5 rounded-lg transition-all duration-500 ${
          ...
          s.status === "healed" ? "bg-white/5 border border-white/[0.08]" :
          "bg-white/[0.03] border border-white/[0.05]"
        }`}>
          <div className="flex items-center gap-2">
            {statusIcon(s.status)}
            <span className="text-xs text-[#050505]/70">{s.name}</span>
          </div>
          <span className={`text-[10px] font-mono ${s.latency > 50 ? "text-red-400" : "text-[#050505]/40"}`}>
            {s.latency}ms
          </span>
        </div>
      ))}
      {isHealing && (
        <motion.div
          ...
          className="text-[10px] text-[#888] text-center py-1.5 bg-white/5 rounded-lg border border-white/[0.08]"
        >
    ...
    <div className="space-y-3">
      ...
      <div className={`rounded-xl p-3 border ${stateBg} transition-all duration-500`}>
        ...
        <div className="grid grid-cols-3 gap-2 text-center">
          <div>
            <div className="text-lg font-bold text-[#050505]/80">{requests}</div>
            <div className="text-[10px] text-[#050505]/40">Requests</div>
          </div>
          <div>
            <div className="text-lg font-bold text-emerald-400">{successes}</div>
            <div className="text-[10px] text-[#050505]/40">Success</div>
          </div>
          <div>
            <div className="text-lg font-bold text-red-400">{failures}</div>
            <div className="text-[10px] text-[#050505]/40">Failures</div>
          </div>
        </div>
      </div>
      <div className="text-[10px] text-[#050505]/30 text-center">
...
function MetricsWidget() {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 text-xs text-[#050505]/50 mb-2">
      ...
      <div className="space-y-2.5">
        <div>
          <div className="flex justify-between text-[10px] mb-1">
            <span className="text-[#050505]/50">CPU</span>
            <span className="text-[#050505]/70 font-mono">{Math.round(metrics.cpu)}%</span>
          </div>
          <GaugeBar value={metrics.cpu} max={100} color={metrics.cpu > 80 ? "bg-red-500" : "bg-[#0a0a0a]"} />
        </div>
        <div>
          <div className="flex justify-between text-[10px] mb-1">
            <span className="text-[#050505]/50">Memory</span>
            <span className="text-[#050505]/70 font-mono">{Math.round(metrics.memory)}%</span>
          </div>
          <GaugeBar value={metrics.memory} max={100} color={metrics.memory > 75 ? "bg-amber-500" : "bg-emerald-500"} />
        </div>
        <div>
          <div className="flex justify-between text-[10px] mb-1">
            <span className="text-[#050505]/50">Latency</span>
            <span className="text-[#050505]/70 font-mono">{Math.round(metrics.latency)}ms</span>
          </div>
          <GaugeBar value={metrics.latency} max={200} color={metrics.latency > 100 ? "bg-red-500" : "bg-sky-500"} />
        </div>
      </div>
      <div className="grid grid-cols-2 gap-2 pt-1">
        <div className="bg-white/[0.03] rounded-lg p-2 text-center border border-white/[0.05]">
          <div className="text-sm font-bold text-[#050505]/80">{metrics.requests.toLocaleString()}</div>
          <div className="text-[10px] text-[#050505]/40">Requests</div>
        </div>
        <div className="bg-white/[0.03] rounded-lg p-2 text-center border border-white/[0.05]">
          <div className="text-sm font-bold text-red-400">{metrics.errors}</div>
          <div className="text-[10px] text-[#050505]/40">Errors</div>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] text-[#050505]/30">Run #{runCount}</span>
      </div>
      {chain.map((c, i) => (
        <div key={i} className={`flex items-center justify-between py-1.5 px-2.5 rounded-lg transition-all duration-300 ${
          ...
          <div className="flex items-center gap-2">
            ...
            {c.status === "idle" && <CircleDot className="w-3 h-3 text-[#050505]/20" />}
            <span className="text-xs text-[#050505]/60 font-mono">{c.model}</span>
          </div>
          {c.latency > 0 && (
            <span className="text-[10px] font-mono text-[#050505]/30">{c.latency}ms</span>
          )}
        </div>
      ))}
      {result && (
  ...
  return (
    <div className="flex flex-col items-center gap-3 py-4">
      <motion.div
        ...
        className="relative"
      >
        <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-cyan-500/20 to-sky-500/20 border border-white/[0.08] flex items-center justify-center">
      ...
      <div className="text-center">
        <div className="text-xs font-semibold text-[#050505]/80">Trust Layer Verified</div>
        <div className="text-[10px] text-[#050505]/40 mt-0.5">Self-healing • Auto-renewing</div>
      </div>
...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      onClick={handleCopy}
      className="absolute top-3 right-3 p-1.5 rounded-lg bg-white/5 hover:bg-white/10 border border-white/[0.08] transition-all"
    >
      {copied ? <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5 text-[#050505]/40" />}
    </button>
  );
}
...
export default function LumeLibrary() {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-foreground overflow-x-hidden">
      ...
      {/* Header */}
      <header className="sticky top-0 z-50 backdrop-blur-2xl bg-[#050505]/60 border-b border-white/[0.05]">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-4 lg:py-5 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link href="/lume" className="w-10 h-10 rounded-xl bg-white/5 border border-white/[0.08] flex items-center justify-center hover:bg-white/10 hover:border-primary/50 transition-all duration-300">
              <ArrowLeft className="w-5 h-5" />
            </Link>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-white/5 border border-white/[0.08] flex items-center justify-center shadow-[0_10px_30px_rgba(0,0,0,0.5)] shadow-cyan-500/25">
                <Boxes className="w-5 h-5 text-[#050505]" />
              </div>
              <div>
                ...
                <span className="hidden lg:inline text-xs text-[#050505]/30 ml-3">Lume Self-Healing Components</span>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Link
              ...
              className="hidden sm:inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 border border-white/[0.08] text-sm text-[#050505]/60 hover:text-[#050505] hover:bg-white/10 transition-all"
            >
            ...
            <a
              ...
              className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/5 border border-white/[0.08] text-[#050505] text-sm font-semibold shadow-[0_10px_30px_rgba(0,0,0,0.5)] shadow-cyan-500/25 hover:shadow-[0_10px_30px_rgba(0,0,0,0.5)] transition-all hover:scale-105"
            >
      ...
      <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-20">
        ...
        >
          <div className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-gradient-to-r from-cyan-500/20 to-teal-500/20 border border-white/[0.08] text-sm font-semibold text-[#888] mb-8 shadow-[0_10px_30px_rgba(0,0,0,0.5)] shadow-cyan-500/10">
          ...
          <div className="flex flex-wrap justify-center gap-3 mt-8">
            <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/5 border border-white/[0.08] text-xs text-[#050505]/50">
          ...
          {widgets.map(w => (
            <button
              ...
              className={`inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-300 ${
                activeWidget === w.id
                  ? "bg-gradient-to-r from-cyan-500/15 to-teal-500/15 border border-white/[0.08] text-[#888] shadow-[0_10px_30px_rgba(0,0,0,0.5)] shadow-cyan-500/10"
                  : "bg-white/5 border border-white/[0.08] text-[#050505]/50 hover:text-[#050505]/70 hover:bg-[#050505]/8"
              }`}
            >
        ...
        >
          ...
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 mb-6">
            <div>
              <h2 className="text-2xl lg:text-3xl font-display font-bold text-[#050505]/90 flex items-center gap-3">
                <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${active.gradient} flex items-center justify-center shadow-[0_10px_30px_rgba(0,0,0,0.5)]`}>
                  <active.icon className="w-5 h-5 text-[#050505]" />
                </div>
                {active.name}
              </h2>
              <p className="text-sm text-[#050505]/50 mt-2 max-w-xl">{active.description}</p>
            </div>
            <div className="flex items-center gap-2">
              {active.tags.map(tag => (
                <span key={tag} className="px-2.5 py-1 rounded-lg bg-white/5 border border-white/[0.08] text-[10px] font-mono text-[#888]">
                  {tag}
                </span>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <GlassCard className="p-6 lg:p-8">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2 text-xs text-[#050505]/40">
            ...
            <GlassCard className="p-0 overflow-hidden">
              ...
              <div className="relative">
                <pre className="p-4 text-[11px] leading-relaxed font-mono text-[#050505]/60 overflow-x-auto max-h-[400px] overflow-y-auto">
        ...
        >
          <h2 className="text-2xl lg:text-4xl font-display font-bold text-center mb-4">
            <span className="bg-white/5 border border-white/[0.08] bg-clip-text text-transparent">
              Why Self-Healing?
            </span>
          </h2>
            ...
            {[
              ...
              <motion.div key={i} variants={staggerItem}>
                <GlassCard className="p-6 h-full">
                  <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${f.gradient} flex items-center justify-center mb-4 shadow-[0_10px_30px_rgba(0,0,0,0.5)]`}>
                    <f.icon className="w-5 h-5 text-[#050505]" />
                  </div>
                  <h3 className="font-display font-semibold text-[#050505]/90 mb-2">{f.title}</h3>
                  <p className="text-xs text-[#050505]/50 leading-relaxed">{f.desc}</p>
                </GlassCard>
              </motion.div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <GlassCard key={i} glow className="p-6 text-center relative overflow-hidden">
                <div className="absolute top-3 right-4 text-5xl font-display font-bold text-[#050505]/[0.03]">{s.step}</div>
                <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-cyan-500/15 to-teal-500/15 border border-white/[0.08] flex items-center justify-center mx-auto mb-4">
                  <s.icon className="w-6 h-6 text-[#888]" />
                </div>
                <h3 className="font-display font-semibold text-[#050505]/90 mb-2">{s.title}</h3>
                <p className="text-xs text-[#050505]/50 leading-relaxed max-w-xs mx-auto">{s.desc}</p>
              </GlassCard>
            ))}
          </div>
        </motion.section>
        ...
        >
          <GlassCard glow className="p-8 lg:p-12 max-w-2xl mx-auto">
            <Sparkles className="w-8 h-8 text-[#888] mx-auto mb-4" />
            <h2 className="text-xl lg:text-3xl font-display font-bold mb-3">
              Build with Lume
            </h2>
            <p className="text-sm text-[#050505]/50 mb-6 max-w-md mx-auto">
            ...
            <div className="flex flex-wrap justify-center gap-3">
              <Link
                href="/lume/playground"
                className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white/5 border border-white/[0.08] text-[#050505] font-semibold shadow-[0_10px_30px_rgba(0,0,0,0.5)] shadow-cyan-500/25 hover:shadow-[0_10px_30px_rgba(0,0,0,0.5)] transition-all hover:scale-105"
              >
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white/5 border border-white/[0.08] text-[#050505]/70 font-semibold hover:bg-white/10 transition-all"
              >
