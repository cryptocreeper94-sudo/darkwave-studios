...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      className="p-1.5 rounded-lg bg-white/5 hover:bg-white/10 border border-white/[0.08] transition-all"
    >
  ...
  return (
    <div className="mb-8">
      <div className="flex items-center justify-between mb-4 px-1">
        <h3 className="text-lg font-display font-bold text-[#050505]/90">{title}</h3>
        <div className="flex items-center gap-1">
          <span className="text-xs text-[#050505]/30 mr-2">{items.length} items</span>
          <button
            onClick={() => scroll(-1)}
            disabled={!canScrollLeft}
            className="w-8 h-8 rounded-lg bg-white/5 border border-white/[0.08] flex items-center justify-center hover:bg-white/10 transition-all disabled:opacity-20"
          >
            ...
            <GlassCard className="p-5 h-full rounded-xl hover:border-primary/30 transition-all duration-300 group">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h4 className="text-sm font-semibold text-[#050505]/90 line-clamp-1 group-hover:text-primary transition-colors">{item.title}</h4>
                    <span className="text-[10px] text-[#050505]/30 font-mono">{item.language}</span>
                  </div>
                </div>
                {item.isPremium ? (
              ...
              <p className="text-xs text-[#050505]/50 leading-relaxed line-clamp-2 mb-3">{item.description}</p>
              <div className="flex flex-wrap gap-1.5">
                {item.tags.slice(0, 3).map(tag => (
                  <span key={tag} className="px-2 py-0.5 rounded-md bg-white/5 border border-white/8 text-[10px] text-[#050505]/40">{tag}</span>
                ))}
                {item.tags.length > 3 && (
                  <span className="text-[10px] text-[#050505]/20">+{item.tags.length - 3}</span>
                )}
              </div>
            </GlassCard>
          </motion.div>
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="w-full max-w-2xl max-h-[85vh] overflow-y-auto rounded-2xl bg-[#0d1117] border border-white/[0.08] shadow-[0_10px_30px_rgba(0,0,0,0.5)]"
      >
        {/* Header */}
        <div className="sticky top-0 bg-[#0d1117]/95 backdrop-blur-xl border-b border-white/[0.05] p-4 flex items-center justify-between z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-display font-bold text-[#050505]/90">{snippet.title}</h2>
              <div className="flex items-center gap-2 text-xs text-[#050505]/30">
          ...
          <button onClick={onClose} className="w-8 h-8 rounded-lg bg-white/5 border border-white/[0.08] flex items-center justify-center hover:bg-white/10 transition-all">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="p-4 space-y-4">
          {/* Description */}
          <p className="text-sm text-[#050505]/60 leading-relaxed">{snippet.description}</p>

          {/* Tags + Price */}
          <div className="flex items-center justify-between">
            <div className="flex flex-wrap gap-1.5">
              {snippet.tags.map(tag => (
                <span key={tag} className="px-2.5 py-1 rounded-lg bg-white/5 border border-white/[0.08] text-[10px] font-mono text-[#888]">{tag}</span>
              ))}
            </div>
            {snippet.isPremium ? (
          ...
          <div className="relative rounded-xl overflow-hidden border border-white/[0.05]">
            <div className="flex items-center justify-between px-4 py-2.5 bg-white/[0.03] border-b border-white/[0.05]">
              <span className="text-xs text-[#050505]/30 font-mono">{snippet.title.toLowerCase().replace(/\s+/g, '-')}.{snippet.language === "typescript" ? "tsx" : "js"}</span>
              <CopyButton text={snippet.code} />
            </div>
            <pre className="p-4 text-[11px] leading-relaxed font-mono text-[#050505]/60 overflow-x-auto max-h-[350px] overflow-y-auto">
          ...
          <div className="rounded-xl bg-white/[0.02] border border-white/[0.05] p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-semibold text-[#050505]/60">Quick Embed</span>
            </div>
    ...
    <div className="min-h-screen bg-[#050505] text-foreground overflow-x-hidden">
      ...
      <header className="sticky top-0 z-50 backdrop-blur-2xl bg-[#050505]/60 border-b border-white/[0.05]">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link href="/developers" className="w-9 h-9 rounded-xl bg-white/5 border border-white/[0.08] flex items-center justify-center hover:bg-white/10 transition-all">
              <ArrowLeft className="w-4 h-4" />
            </Link>
            <div className="flex items-center gap-2.5">
              <div className="w-9 h-9 rounded-xl bg-white/5 border border-white/[0.08] flex items-center justify-center shadow-[0_10px_30px_rgba(0,0,0,0.5)] shadow-[0_10px_30px_rgba(255,255,255,0.05)]">
                <Package className="w-4.5 h-4.5 text-[#050505]" />
              </div>
              <div>
                <span className="font-display text-base lg:text-lg font-bold text-[#050505]/90">Marketplace</span>
                <span className="hidden sm:inline text-xs text-[#050505]/30 ml-2">{totalCount} widgets</span>
              </div>
            </div>
          </div>
          <Link
            href="/lume/library"
            className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl bg-gradient-to-r from-cyan-500/10 to-teal-500/10 border border-white/[0.08] text-xs text-[#888] font-semibold hover:bg-[#050505]/15 transition-all"
          >
      ...
      <main className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
        ...
        <motion.section initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          ...
          <p className="text-sm text-[#050505]/50 max-w-lg mb-6">
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/5 border border-white/8 text-xs text-[#050505]/50">
              <Package className="w-3 h-3 text-[#888]" /> {totalCount} Total
            </div>
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/5 border border-white/8 text-xs text-[#050505]/50">
        ...
        <div className="mb-8 space-y-4">
          ...
          <div className="relative max-w-md">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#050505]/30" />
            <input
              type="text"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder="Search widgets, hooks, components..."
              className="w-full bg-white/[0.03] border border-white/[0.08] rounded-xl pl-10 pr-4 py-2.5 text-sm text-[#050505] placeholder-white/25 focus:border-white/[0.08] focus:outline-none transition-colors"
              data-testid="search-input"
            />
            {searchQuery && (
              <button onClick={() => setSearchQuery("")} className="absolute right-3 top-1/2 -translate-y-1/2">
                <X className="w-3.5 h-3.5 text-[#050505]/30 hover:text-[#050505]/60" />
              </button>
            )}
          </div>
              ...
              <button
                ...
                className={`flex-shrink-0 inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-medium transition-all ${
                  activeCategory === cat.id
                    ? "bg-gradient-to-r from-cyan-500/15 to-sky-500/15 border border-white/[0.08] text-[#888]"
                    : "bg-white/[0.03] border border-white/8 text-[#050505]/40 hover:text-[#050505]/60 hover:bg-[#050505]/[0.06]"
                }`}
        ...
        {loading && (
          <div className="text-center py-20">
            <div className="w-8 h-8 border-2 border-white/[0.08] border-t-cyan-500 rounded-full animate-spin mx-auto mb-3" />
            <p className="text-sm text-[#050505]/30">Loading marketplace...</p>
          </div>
        )}
        ...
        {!loading && (
          <>
            {activeCategory === "all" && !searchQuery ? (
              ...
              /* Filtered Grid View */
              <>
                <p className="text-xs text-[#050505]/30 mb-4">{filtered.length} result{filtered.length !== 1 ? "s" : ""}</p>
                  ...
                  {filtered.map(item => (
                    ...
                    >
                      <GlassCard className="p-5 h-full rounded-xl hover:border-primary/30 transition-all group">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {item.tags.slice(0, 3).map(tag => (
                            <span key={tag} className="px-2 py-0.5 rounded-md bg-white/5 text-[10px] text-[#050505]/40">{tag}</span>
                          ))}
                        </div>
                      </GlassCard>
                    </motion.div>
            ...
            {filtered.length === 0 && !loading && (
              <div className="text-center py-16">
                <Search className="w-10 h-10 text-[#050505]/10 mx-auto mb-3" />
                <h3 className="text-lg font-display font-bold text-[#050505]/40 mb-1">No widgets found</h3>
                <p className="text-sm text-[#050505]/25">Try a different search or category</p>
              </div>
            )}
          </>
        )}
