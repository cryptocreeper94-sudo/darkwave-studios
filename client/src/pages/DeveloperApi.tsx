...
import { 
  Code2, Shield, Zap, Lock, Globe, Bot, ChevronRight, Copy, Check, Activity,
  TrendingUp, BarChart3, Clock, ArrowLeft, ExternalLink, Terminal, Megaphone,
  Target, Users, Building2, Wrench, Home, Mail, Newspaper, Sparkles, Boxes
} from "lucide-react";
      ...
      <GlassCard glow className="rounded-2xl lg:rounded-3xl p-8 lg:p-12 bg-gradient-to-br from-amber-500/5 via-transparent to-orange-500/5">
        ...
        <div className="bg-black/40 rounded-xl p-4 mb-8 border border-amber-500/20">
          <div className="flex items-center justify-between mb-2">
            ...
            <button onClick={() => copyToClipboard("https://darkwavepulse.com/api", "baseurl")} className="flex items-center gap-1.5 text-xs text-amber-400 hover:text-amber-300 transition-colors" data-testid="copy-base-url">
            ...
            <div key={endpoint.path} className="bg-black/30 rounded-xl border border-white/10 overflow-hidden" data-testid={`endpoint-${endpoint.path.replace(/\//g, '-')}`}>
              <div className="p-4 border-b border-white/10">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <button onClick={() => copyToClipboard(endpoint.path, endpoint.path)} className="ml-auto text-muted-foreground hover:text-white transition-colors" data-testid={`copy-${endpoint.path.replace(/\//g, '-')}`}>
  ...
  return (
    <div className="min-h-screen bg-background text-foreground overflow-x-hidden">
      <SEOHead
        ...
        keywords="developer tools, API, Pulse API, press, outreach"
      />
      <BreadcrumbSchema
        items={[
          { name: "Home", url: "https://darkwavestudios.io" },
          { name: "Developers", url: "https://darkwavestudios.io/developers" },
          { name: "API & Tools", url: "https://darkwavestudios.io/developers/api" }
        ]}
      />
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-3 lg:py-4 flex items-center justify-between">
          <div className="flex items-center gap-3 lg:gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-display text-lg lg:text-xl font-bold">API & Developer Tools</span>
            </div>
          </div>
          <Link href="/hub" className="btn-glow bg-primary hover:bg-primary/90 text-white px-4 py-2 rounded-lg text-sm font-semibold transition-colors" data-testid="button-browse-widgets">
            Browse Widgets
          </Link>
        </div>
      </header>
            ...
            <div className="space-y-8">
              ...
              <div>
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {[
                    { name: "TechCrunch", url: "https://techcrunch.com/submit-a-tip/", type: "Tip submission form", pitch: "Full-stack ecosystem with 37 production apps and 1.86M+ lines of code", icon: Zap },
                    ...
                    { name: "Hacker News (Show HN)", url: "https://news.ycombinator.com/submit", type: "Community submission", pitch: "Technical deep-dive on building a 37-app ecosystem as a solo developer", icon: Terminal },
              ...
              <div>
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {[
                    ...
                    { name: "Decrypt", url: "https://decrypt.co/contact", type: "Story pitches", pitch: "Real-world blockchain utility — 37 apps verified through Trust Layer", icon: Boxes },
              ...
              <div>
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {[
                    { name: "Painting Contractor Magazine", url: "https://www.paintmag.com", type: "Trade publication", pitch: "PaintPros.io + TradeWorks AI — 85+ calculators, voice-to-estimate, AI tools for painters", icon: Wrench },
                    { name: "Staffing Industry Analysts", url: "https://www2.staffingindustry.com", type: "Staffing industry news", pitch: "ORBIT Staffing OS — automated white-label staffing with GPS tracking, payroll, compliance", icon: Users },
                    { name: "Inman News", url: "https://www.inman.com/contribute/", type: "Real estate tech", pitch: "TrustHome — Expo React Native platform with Voice AI, blockchain doc vault, MLS integration", icon: Home },
                    { name: "HousingWire", url: "https://www.housingwire.com", type: "Real estate + fintech", pitch: "Blockchain-verified real estate transactions with TrustHome + Trust Layer", icon: Building2 },
              ...
              <div>
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {[
                    { name: "SaaStr", url: "https://www.saastr.com", type: "SaaS community + events", pitch: "37-app ecosystem with widget marketplace, subscription model, and AI credits system", icon: TrendingUp },
              ...
              <div className="mt-8 p-6 rounded-xl bg-gradient-to-r from-primary/5 to-accent/5 border border-primary/10">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <div className="space-y-3">
                    {[
                      "Lead with the story — solo developer building a 37-app, 1.86M+ line ecosystem is compelling and rare",
                  ...
                  <div className="space-y-3">
                    {[
                      "For trade publications, offer to write a guest article — position yourself as a tech expert solving industry problems",
                      "Include live demo links and the Ecosystem Metrics page — real numbers build credibility instantly",
                      "Follow up once after 5–7 days. Keep pitches under 200 words. Journalists are busy — make it easy to say yes",
                    ].map((tip, i) => (
                      <div key={i} className="flex items-start gap-2">
                        <div className="w-5 h-5 rounded-full bg-primary/20 flex items-center justify-center flex-shrink-0 mt-0.5">
                          <span className="text-[10px] font-bold text-primary">{i + 4}</span>
                        </div>
                        <p className="text-sm text-muted-foreground">{tip}</p>
                      </div>
                    ))}
        ...
        >
          <GlassCard glow className="rounded-2xl lg:rounded-3xl p-8 lg:p-12">
            ...
            <div className="flex flex-wrap justify-center gap-4">
              <Link href="/hub" className="btn-glow inline-flex items-center gap-2 bg-primary text-white px-8 py-4 rounded-xl font-semibold hover:opacity-90 transition-opacity" data-testid="button-cta-widgets">
